...
struct waitqueue* jobselect()
{
	...
	int highest = current?current->job->curpri:-1;

	//added
	int most_time=current?current->job->wait_time:-1;
	//added
	...
	if (head) {
		for (prev = head, p = head; p != NULL; prev = p, p = p->next) {
			...
			if (p->job->curpri > highest) {
				...
				//added
				most_time = p->job->wait_time;
			}else if(p->job->curpri == highest && p->job->wait_time > most_time){
				select = p;
				selectprev = prev;
				most_time = p->job->wait_time;
				//added
...
void sig_handler(int sig, siginfo_t *info, void *notused)
{
	...
	case SIGCHLD:
		...
		//do_stat();
			...
			if(current->job->pid == info->si_pid){
				select = current;
				current->job->state = DONE;
			}else if (head) {
				for (prev = head, p = head; p != NULL; prev = p, p = p->next) {
					if(p->job->pid==info->si_pid){
						select = p;
						selectprev = prev;
						break;
					}
			...
			printf("abnormal termation, signal number = %d\tjid = %d, pid = %d\n\n",
				WTERMSIG(status), select->job->jid, select->job->pid);
...
void do_deq(struct jobcmd deqcmd)
{
	...
	if (current && current->job->jid == deqid) {
		...
		if (head) {
			...
			//look the line 396, it prevents both select and head from being null at the same time.
			if(select==head){
				head = select->next;
			}else if(select){
			//added
...
void do_stat()
{
	...
	*/
	...
	printf( "JOBNAME\tJID\tPID\tOWNER\tCURPRI\tDEFPRI\tRUNTIME\tWAITTIME\tCREATTIME\t\tSTATE\n");
